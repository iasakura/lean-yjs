...
- さらに fresh-id 仮定の供給元も spec 側の前提までつないだ
  - `IntegrateInput.toItemV2_id_eq`
  - `ofOldItems_lookup_none_of_isClockSafe`
  - `SpecPortV2.activeSetV2_structural_of_toItemV2_isClockSafe`
  - `SpecBridgeV2.activeSetV2_structural_of_toItem_maximalId`
  - これで old spec の `maximalId` 仮定から、proof-local active set の structural hypothesis を直接作れる
  - 次の本丸は、この structural hypothesis を実際に使って `loopInvV2` の order 補題を移植すること
...
`Spec` の v2 直移植へ pivot した後の到達点を固定しておく。

### Order monotonicity under `withItem`

`LeanYjs/Order/ItemOrderV2.lean` で、old item-set 上の order を
candidate 追加後の active set へ持ち上げる補題が入った。

- `originLtV2_withItem`
- `yjsLtV2_withItem`
- `yjsLeqV2_withItem`
- `conflictLtV2_withItem`
- `yjsLtV2'_withItem`
- `yjsLeqV2'_withItem`

ここは theorem recursion ではなく mutual inductive recursor (`YjsLtV2.recOn`) を使う形にした。
proof-term の `sizeOf` termination を押し切るより、こちらの方が安定している。

### Active-set order wrappers

`LeanYjs/Algorithm/Insert/SpecPortV2.lean` 側では、old item-set で得た order / totality を
`activeSetV2 = ofOldItems + withItem newItem` へ持ち上げる thin wrapper が入った。

- `activeSetV2_yjsLt_of_old`
- `activeSetV2_yjsLeq_of_old`
- `activeSetV2_yjsLeq_or_yjsLt_of_oldRefIn`
- `activeSetV2_yjsLeq_or_yjsLt_of_oldRefIn_of_toItemV2_isClockSafe`

これで「old ref 同士の比較」は、`activeSetV2` を使う `loopInvV2` 側へ
old theorem 非依存で差し替え始められる。

### Reachability lowering under `withItem`

`LeanYjs/Order/ItemOrderV2.lean` で、`withItem` した active set 上の reachability を
old item-set 側へ降ろす基盤を追加した。

- `originReachableStepV2_of_withItem_of_ne`
- `originReachableV2_of_withItem_of_ne`

`LeanYjs/Algorithm/Insert/SpecPortV2.lean` 側にも対応 wrapper を追加した。
...
まだ未解決なのは `newItem` 自身を片側に含む比較である。

- old ref vs old ref: 既に `activeSetV2` へ lift できる
- newItem vs old ref
- old ref vs newItem
- newItem.origin / newItem.rightOrigin を経由する局所補題
- newItem 自身を start にした reachability を
  `OriginReachableFromV2` / validity とどう繋ぐか

つまり、`loopInv_YjsLt'` 全体の移植前に必要なのは
「candidate を片側に持つ order 補題」の v2-native 化である。

### Candidate-side reachability bridge

その後、candidate item 自身を start にした reachable path を
`activeSetV2` から old item-set 側へ落とす補題も追加した。

- `originReachableFromV2_of_withItem`
- `IsItemValidV2Against.reachable_YjsLeq_fromV2AgainstOldItems`
- `activeSetV2_origin_lt_rightOrigin_of_valid`
- `activeSetV2_origin_nearest_reachable_of_valid`

これで `newItem` branch についても、

- `activeSetV2` 上の path
- `OriginReachableFromV2 (ofOldItems ...) newItem ...`
- old validity / nearest-reachable
- `activeSetV2` 上の `YjsLeqV2'`

という経路で戻せるようになった。

したがって `SpecPortV2` 側の残課題は、
candidate-side invariant そのものではなく、
`loopInv_YjsLt'` 内で使っている局所的な order constructor
(`ltOrigin`, `ltOriginOrder`, `ConflictLt'.ltOriginSame` など) を
`activeSetV2` 上で組み直す段階へ移った。

### Immediate next target

次にやるべきことはこれ。

1. old `loopInv_YjsLt'` の proof を分解して、
   `ltOrigin` / `ltOriginOrder` / `ConflictLt'.ltOriginSame`
   で閉じている枝を `SpecPortV2` に順に移す
2. `newItem` と old item の比較について、
   `activeSetV2_origin_lt_rightOrigin_of_valid` /
   `activeSetV2_origin_nearest_reachable_of_valid` を使う thin wrapper を足す
3. その上で `loopInv_YjsLt'` の v2 版本体へ入る

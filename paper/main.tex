...
%% P1: Motivation
Collaborative editors must reconcile insertions and deletions performed at
different replicas without requiring a single global order of user actions.
Yjs~\cite{Yjs2023} is one of the most widely deployed CRDT implementations
for this setting, powering collaborative editors such as
Tiptap~\cite{TipTap2024}.  Its core algorithm, however, is intricate, and
reasoning about its correctness by hand is error-prone: while developing
the present formalization we discovered a non-convergent case in the
pseudocode published in the YATA paper of Nicolaescu et
al.~\cite{Nicolaescu2016} on which Yjs is based.  The original author
confirmed our finding---``Nine years later, you are actually the first to
point out the issue in the pseudocode in the
paper''~\cite{YjsBugDiscussion2024}---which itself illustrates why
machine-checked correctness proofs are valuable for CRDTs of this
complexity.

%% P2: Yjs algorithm intuition
Yjs represents each inserted character as an item whose identifier is unique
and whose \emph{origin} and \emph{right origin} record the neighboring items
seen when the character was created.  When two replicas insert into the same
interval concurrently, the integration procedure scans the items between
these two recorded neighbors and uses a deterministic tiebreaker to choose a
single position.  This design differs from sequence CRDTs such as
RGA~\cite{Roh2011} and Logoot~\cite{Weiss2010}, where globally unique
timestamps or dense position identifiers make the ordering of concurrent
insertions independent of the current document state.  In Yjs, the
correctness argument must instead explain why each insertion is applied only
to states in which its recorded neighbors still form a structurally valid
interval.

%% P3: Prior work and the gap
No complete machine-checked convergence proof has been published for the
Yjs core algorithm.  Among Yjs-specific developments, Nicolaescu et
al.~\cite{Nicolaescu2016} introduced YATA with hand proofs for selected
properties only, and Weidner et al.'s Fugue~\cite{Weidner2023}---a closely
related sequence CRDT---accompanies its design with a by-hand argument for
a variant of the Yjs insertion procedure.  Generic CRDT frameworks
naturally raise the question whether one of them already subsumes Yjs.
Shapiro et al.~\cite{Shapiro2011} formulate CmRDTs in a way that
accommodates state-dependent effectors: an effector is enabled only when a
delivery precondition holds.  Existing \emph{mechanized} convergence
frameworks, however, have typically avoided this enabledness obligation by
proving convergence from \emph{unconditional} commutativity.  In the
Isabelle/HOL framework of Gomes et al.~\cite{Gomes2017}, for example,
validity appears at the network level, but the convergence theorem itself
does not need to transport validity across the intermediate states
encountered by an adjacent-swap proof.

%% P4: Why it fails for Yjs
Yjs exposes exactly the gap between these two styles of reasoning.  We
exhibit a mechanized counterexample (\S\ref{sec:overview}) showing that two
Yjs insert operations need not commute when they are applied to an arbitrary
state.  The counterexample is unreachable under causal delivery, but
unconditional commutativity quantifies over all states, including states in
which an operation's origin and right origin no longer describe a valid
interval.  The failure is therefore not a convergence bug in the corrected
Yjs algorithm; it shows that the usual mechanized proof principle is too
strong for this state-dependent insertion procedure.

%% P5: Our solution — prefix-level conditional commutativity + causal validity
Our proof follows the Shapiro-style idea of restricting commutativity to
enabled states, but makes the corresponding delivery obligation precise
enough for mechanization.  The generic convergence theorem
(Theorem~\ref{thm:generic-conv}) requires conditional commutativity on
states satisfying an invariant, together with a prefix-level causal
validity condition: whenever a $\hb$-consistent, $\hb$-closed,
duplicate-free prefix already contains the $\hb$-predecessors of an
operation, that operation is valid at the state obtained by applying
the prefix.  This condition is what lets an adjacent-swap proof apply
an operation at intermediate states other than the one at which it was
originally broadcast.  Instantiating the theorem for Yjs gives the first
complete machine-checked proof of strong eventual consistency for the
Yjs core CRDT algorithm (Theorem~\ref{thm:yjs-convergence}).
...
\item \textbf{A prefix-level conditional commutativity framework.}
  ...
  delivery-precondition obligation implicit in Shapiro-style CmRDTs,
...
swap-based commutativity argument.
...
hypotheses---unconditional concurrent commutativity---is too strong for
...
concurrent operations $a$ and $b$ and every state $s$,
\[
...
containing $a$ in this configuration; the counterexample is unreachable.
...
Following the Shapiro-style CmRDT~\cite{Shapiro2011} idea of restricting
...
convergence argument relies on.  The list must be consistent with the
...
The convergence argument reorders operations by adjacent swaps, so we
...
This $\hb$-monotone character is exactly what the swap argument needs:
...
$\YjsLt$ on items that underlies the rest of the argument;
...
and the algorithmic argument rely.  In the counterexample, item~$o$ has
...
precondition under which the commutativity argument operates.  The
...
insert preservation is a loop-invariant argument on
...
We isolate the structural argument that two concurrent inserts do not
disturb each other's validity as a separate lemma, since it is the only
...
definition not in $a$'s broadcast prefix; the detailed argument is
...
This appendix supplies the omitted argument from the proof sketch of
...
origin-reachable from $a$'s item.  The symmetric argument shows that
$a$'s item is not origin-reachable from $b$'s item.  These are exactly

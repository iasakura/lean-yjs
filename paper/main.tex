...
\title{A Mechanized Convergence Proof for the Yjs CRDT}

\begin{abstract}
Yjs is one of the most widely deployed CRDTs for collaborative sequence
editing, yet its core algorithm has lacked a complete formal correctness
proof.  We present the first machine-checked proof that the Yjs core
CRDT algorithm achieves strong eventual consistency under causal
delivery, mechanized in Lean~4.  The main lemma establishes that Yjs
insert and delete preserve a strict total order on items; for the
non-trivial insert procedure, we identify a four-part loop invariant
that drives the preservation proof.  Standard op-based CRDT convergence
theorems, including that of Gomes et al., do not apply to Yjs: we
exhibit a mechanized counterexample showing that Yjs operations are
not unconditionally commutative.  Convergence is therefore obtained via
a generic \emph{conditional commutativity} framework we develop along
the way, pairing a state-dependent commutativity premise with a
prefix-level causal validity transport condition.  The latter refines
the delivery-precondition obligation implicit in Shapiro-style op-based
CRDTs.  The framework is potentially reusable for other CRDTs whose
effectors are well-defined only on invariant-satisfying states.  The
entire development uses no axioms beyond those of Lean's type theory.
\end{abstract}
...
%% P1: Motivation
Collaborative editors must reconcile edits such as insertions and
deletions of characters performed at different replicas without
requiring a single global order of user actions.  Conflict-free
replicated data types (CRDTs)~\cite{Shapiro2011} are a class of data
structures designed for this style of replication, in which each
replica processes operations locally and replicas converge to the same
state once they have exchanged all updates.  Yjs~\cite{Yjs2023} is one
of the most widely deployed CRDT implementations for sequence data,
powering collaborative editors such as Tiptap~\cite{TipTap2024}.  Its
core algorithm is intricate, however, and reasoning about its
correctness by hand is error-prone.  Indeed, we discovered a
non-convergent case in the pseudocode of YATA~\cite{Nicolaescu2016},
the algorithm on which Yjs is based.  An author of the paper confirmed
our finding, remarking that ``Nine years later, you are actually the
first to point out the issue in the pseudocode in the
...
addressing Yjs's correctness, Nicolaescu et al.~\cite{Nicolaescu2016}
introduced YATA together with hand proofs of selected properties, and
Weidner et al.'s Fugue~\cite{Weidner2023}, a closely related sequence
CRDT, includes a by-hand proof for a variant of the Yjs insertion
procedure in its appendix.  Both efforts remain at the level of proof
sketches and fall significantly short of a complete formalization.

%% P3: Gomes framework
Gomes et al.~\cite{Gomes2017} proposed a formal verification framework
for operation-based CRDTs in Isabelle/HOL.  Their main theorem
encapsulates the standard sufficient condition for op-based CRDT
convergence: if all concurrent operation pairs commute on every state,
strong eventual consistency follows.  They use the framework to verify
several CRDT algorithms including RGA~\cite{Roh2011}, a list CRDT in
the same family as Yjs.
...
framework requires.  In Gomes's definition, commutativity is
...
list).  Yjs operations commute only when the operations and the state
satisfy a specific structural condition.  Since Gomes's definition
quantifies over operations and states independently, such a relational
constraint cannot be expressed.

%% P5: Main result and proof framework
This work presents the first complete machine-checked proof of strong
eventual consistency for the Yjs core CRDT algorithm
(Theorem~\ref{thm:yjs-convergence}).  To accommodate the failure of
unconditional commutativity, our convergence theorem rests on a
relaxed commutativity premise we call \emph{conditional commutativity}:
$\effect(a, \effect(b, s)) = \effect(b, \effect(a, s))$ is required
only when both $a$ and $b$ are valid at $s$ under a state-dependent
validity predicate specified by the CRDT.  Conditional commutativity
alone, however, is not enough.  The convergence proof reorders one
delivery list into another by repeatedly swapping adjacent concurrent
operations; the swap step requires both operations to be valid at the
current state, which may differ from the one at which they were
originally broadcast.  We therefore additionally require that
broadcast-time validity carry over to any state reached under causal
delivery---a transport property we call \emph{causal validity}.
Together, conditional commutativity and causal validity yield
convergence: causal delivery guarantees that every intermediate state
visited during reordering is reached by a list containing the swapped
operations' causal predecessors, causal validity then transports
broadcast-time validity to that state, and conditional commutativity
uses the resulting validity to justify each swap.

%% P5b: Yjs proof strategy
The bulk of the development establishes these premises for Yjs.  For
conditional commutativity, we define a strict total order on Yjs items
and show that every Yjs operation preserves the invariant that the
array is sorted by this order; since a strict total order admits at
most one sorted arrangement of any item set, two delivery orders
producing the same item set yield identical arrays.  The overall
strategy follows that of Nicolaescu et al.~\cite{Nicolaescu2016}, but
the order definition and the preservation proof are new.  Insert
preservation is proved by a loop invariant with intricate conditions;
the inductive step reasons about six independent cases, depending on
which branch the algorithm takes and on the value of the scanning
flag.  Causal validity follows because Yjs item validity depends only
on which items are present in the state, not on the order in which
the operations producing them were applied.  The order definition,
the loop invariant, the six-case preservation analysis, and the
causal-validity proof are all machine-checked in Lean~4, ruling out
case omissions in the analysis of such an intricate algorithm.

%% P6: Contributions
This work makes two contributions.
\begin{enumerate}
\item \textbf{First mechanized convergence proof for the Yjs core CRDT
  algorithm.}
  We provide the first machine-checked proof, in Lean~4, that the Yjs
  core CRDT algorithm achieves strong eventual consistency under causal
  delivery.  The proof rests on a strict total order on Yjs
  items---with totality, transitivity, and asymmetry established by
  dedicated proofs---together with a no-cross-origin lemma that
  constrains the origin pointers of comparable items; the inference
  rules of the order are carefully balanced so that transitivity and
  asymmetry can be proved by size-based induction.  The main lemma is
  that Yjs insert and delete preserve this order on the items array.
  Insert preservation is the technical core: the procedure is defined
  by a non-trivial procedural loop, and proving order preservation
  required discovering a four-part loop invariant for the integration
  scan.
\item \textbf{A conditional commutativity convergence framework.}
  We exhibit a mechanized counterexample showing that the unconditional
  commutativity premise of standard op-based CRDT convergence theorems,
  including that of Gomes et al., fails for Yjs.  We then formulate,
  and mechanize in Lean~4, a generic convergence theorem in which
  commutativity is required only when both concurrent operations are
  valid at the current state, paired with a prefix-level causal
  ...
  The latter may be viewed as a mechanized, prefix-level refinement of
  the delivery-precondition obligation implicit in Shapiro-style
  op-based CRDTs.
\end{enumerate}
...
conditional commutativity and gives a high-level overview of the proof.
...
(\leanref{IntegrateError}); we omit this detail here because the error payload plays no
role in the proofs.}
\end{definition}
\noindent
...
\begin{lemma}[Swap preserves consistency and closure,
  \leanref{hb\_consistent\_swap}, \leanref{hbClosed\_swap}]
\label{lem:swap}
...
\item[(I2)] \textbf{ID uniqueness.} No two distinct items in $P$ share the
  same identifier.
\item[(I3)] \textbf{Origin adjacency.} For every
  $x = \langle o, r, \mathit{id}, c \rangle \in P$ and every pointer $z$
  origin-reachable from $x$, either $z \YjsLeq o$ or $r \YjsLeq z$.
\end{itemize}
\end{definition}

Since $o$ and $r$ are themselves origin-reachable from $x$, I1 and I3
together say that $o$ and $r$ are \emph{adjacent} among the pointers
origin-reachable from $x$: they occur in this order, and nothing
reachable from $x$ lies strictly between them.  This is what survives of
$x$'s creation: at the generating replica $o$ and $r$ were the immediate
array neighbors of the insertion position, hence adjacent in the whole
document, and although later concurrent inserts destroy that global
adjacency---placing an item between $o$ and $r$ is exactly what an insert
does---adjacency restricted to what $x$ can reach is preserved.
...
The framework is independent of Yjs: any CRDT whose effectors commute
on invariant-satisfying reachable states, rather than on all states,
fits naturally into our generic theorem
(Theorem~\ref{thm:generic-conv}).
...
The present formalization models the core CRDT algorithm with insert
and delete operations.  We do not model garbage collection, undo/redo,
subdocuments, or the awareness protocol.  Extending the proof to these

...
の2つの補題から、YATAが収束することを証明していますが、1は複雑な定義と式変形によって行われ、
2については証明は一切与えられていません。

そこで私は今年の2月頃からYjsアルゴリズム、特に新規Itemを挿入するintegrateアルゴリズムのコア部分をLean4で形式化・証明するプロジェクトを個人で進めてきました。
難所はあったものの、先日無事に可換性の証明という1つのマイルストーンを達成することができました。
私が示したのは可換性という、Yjsのようなop-basedの正しさの必要条件で知られているものです。

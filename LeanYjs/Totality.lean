...
lemma yjs_lt_total {A : Type} {P : @ClosedPredicate A} {inv : ItemSetInvariant P} {x y : YjsPtr A} (hx : P.val x) (hy : P.val y) :
  ...
  | YjsPtr.itemPtr x =>
    ...
    | YjsPtr.itemPtr y => by
      ...
      obtain hleq := yjs_lt_total hx hyo
      cases hleq with
      | inl hleq =>
        obtain ⟨ h, hleq ⟩ := hleq
        left
        suffices (∃ h, YjsLt P h (YjsPtr.itemPtr (YjsItem.item xo xr xid xc)) (YjsPtr.itemPtr (YjsItem.item yo yr yid yc))) from by
          obtain ⟨ h, hlt ⟩ := this
          exists h
          right
          assumption
      ...
      | inr hltyox =>
        obtain hleq := yjs_lt_total hyr hx
        cases hleq with
        | inl hleq =>
          obtain ⟨ h, hleq ⟩ := hleq
          right
        ...
        | inr hltxyr =>
          obtain hleq := yjs_lt_total hy hxo
          cases hleq with
          | inl hleq =>
            obtain ⟨ h, hleq ⟩ := hleq
            right
            ...
            apply OriginLtStep.lt_left
          | inr hltxoy =>
            obtain hleq := yjs_lt_total hxr hy
            cases hleq with
            | inl hleq =>
              obtain ⟨ h, hleq ⟩ := hleq
              left
            ...
            | inr hltyxr =>
              ...
              obtain hleq := yjs_lt_total hxo hyo
              cases hleq with
              | inr hlt =>

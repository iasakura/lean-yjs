...
theorem no_cross_origin {x y : YjsItem A} :
  ...
  . cases hx
    ...
    cases xr with
    | first =>
      have ⟨ _, horlt ⟩ := hinv.origin_not_leq xo YjsPtr.first xc xid hpx
    ...
    | itemPtr xr =>
      ...
      | inl hleq =>
        cases hleq
        have hreachable : OriginReachable (YjsPtr.itemPtr (YjsItem.mk xo (YjsPtr.itemPtr y) xid xc)) y.origin := by
      ...
      | inr hlt =>
        ...
        | inl hleq =>
          obtain ⟨ h, hleq ⟩ := hleq
          left
          have hreachable : OriginReachable (YjsPtr.itemPtr (YjsItem.mk xo xr xid xc)) xr.origin := by
        ...
        | inr hlt =>
          obtain ⟨ _, hlt ⟩ := hlt
          right
          have ⟨ h', hlt' ⟩ : YjsLt' (YjsPtr.itemPtr (YjsItem.mk xo (YjsPtr.itemPtr xr) xid xc)) xr := by

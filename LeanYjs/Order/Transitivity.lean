...
theorem conflict_lt_trans {A} [DecidableEq A] {P : ItemSet A} {inv : ItemSetInvariant P} :
  ...
  subst heqx heqz
  have hltxrz :
    YjsLeq' xr (YjsItem.mk zo zr zid zc) ∨ YjsLt' (YjsItem.mk zo zr zid zc) xr := by
    apply yjs_lt_total <;> try assumption
    apply hclosed.closedRight; assumption
  have hltxzo :
    YjsLeq' (YjsItem.mk xo xr xid xc) zo ∨ YjsLt' zo (YjsItem.mk xo xr xid xc) := by
    apply yjs_lt_total <;> try assumption
  ...
  have hltxzr : YjsLt' (YjsPtr.itemPtr (YjsItem.mk xo xr xid xc)) zr := by
    obtain ⟨ _, hyzr ⟩ := hyzr
    apply ih (y := y) ((YjsPtr.itemPtr (YjsItem.mk xo xr xid xc)).size + y.size + zr.size) (by simp [YjsItem.size, YjsPtr.size] at *; omega) _ _ hpx hpy (by apply hclosed.closedRight; assumption) <;> try simp
    . apply YjsLt.ltConflict
      assumption
    . assumption

  have hltxoz : YjsLt' xo (YjsPtr.itemPtr (YjsItem.mk zo zr zid zc)) := by
    obtain ⟨ _, hxoy ⟩ := hxoy
    apply ih (y := y) (xo.size + y.size + (YjsPtr.itemPtr (YjsItem.mk zo zr zid zc)).size) (by simp [YjsItem.size, YjsPtr.size] at *; omega) _ _ (by apply hclosed.closedLeft; assumption) hpy hpz <;> try simp
    . assumption
    . apply YjsLt.ltConflict
      assumption
      ...
      | ltOriginDiff h1 h2 h3 h4 l1 l2 r1 r2 id1 id2 c1 c2 hlt1 hlt2 hlt3 hlt4 =>
        ...
        | ltOriginDiff h5 h6 h7 h8 l3 l4 r3 r4 id3 id4 c3 c4 hlt5 hlt6 hlt7 hlt8 =>
          ...
          have hlt : zo.size + l2.size + xo.size <
            (YjsPtr.itemPtr (YjsItem.mk xo xr xid xc)).size +
            (YjsPtr.itemPtr (YjsItem.mk l2 r2 id2 c2)).size +
            (YjsPtr.itemPtr (YjsItem.mk zo zr zid zc)).size := by
  ...
  . subst hyeq
    ...
    . obtain ⟨ o, r, id, c ⟩ := y'
      ...
      have ⟨ hor, horlt ⟩ : YjsLt' o r := by
        apply inv.origin_not_leq (o := o) (r := r) (c := c) (id := id) <;> assumption
      cases hleq with
      | leqSame =>
        cases hleq' with
        | leqSame =>
